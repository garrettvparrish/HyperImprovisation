...
//---------------Var for breath------------
int[] breathList = new int[50];               
int[] minB = new int[5];  
int breath = 0;
boolean bbreath = false;
...
port = new Serial(this, Serial.list()[2], 115200); // choose the right one in square brackets
...
// set the BPM visualizer line to 0
 for (int i=0; i<rateY.length; i++){
   ...
   //------ osc------
    /* start oscP5, listening for incoming messages at port 4662 */
  oscP5 = new OscP5(this, 1201);
  myRemoteLocation = new NetAddress("127.0.0.1", 1201);
}
...
// print out the graph of the heart rate
 stroke(250,250,0);                                     // color of heart rate graph
 ...
 */
 ...
 background(5,5,5);
 if(On){
   output.println(Sensor +","+pulseRate+","+breath+","+PieceOn); // 
   println(Sensor +":"+pulseRate+":"+breath+","+PieceOn);
  background(200,5,5);
 }
